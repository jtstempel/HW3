...
## Kevin Rothstein helped me with the strucutre of the longest_tweet.html template as it wasn't functioning properly for me. 
## Also, for some reason the provided 404 template was blank - so I used similar code from the provided 500 template.
## Ava Weiner helped me update the "possible tweets saved"
...
app.config["SQLALCHEMY_DATABASE_URI"] = 'postgresql://josephtstempel@localhost/jstempelHW3'
## Provided:
...
class TweetEntryForm(FlaskForm):
    text = StringField('Please enter the text of the tweet (no more than 280 characters): ', validators=[Required()])
    username = StringField('Please enter the Twitter username (exclude @ symbol, no more than 64 characters): ', validators=[Required()])
    display_name = StringField('Please enter the display name of the Twitter user (must be a least two words): ', validators=[Required()])
    submit = SubmitField()

    def validate_username(self, field):
        if '@' in self.username.data:
            raise ValidationError('Your username was invalid because it started with an @ symbol')

    def validate_display_name(self, field):
        display_split = self.display_name.data.split()
        if len(display_split)<2:
            raise ValidationError('Your display name was invalid because it was fewer than two words')
...
def index():
    ...
    ## Redirect to the index page
        ...
        db.session.commit()
        flash('This tweet has been successfully added')
...
def see_all_tweets():
    ...
    all_tweets = []
    for some_tweet in tweets:
        all_tweets.append([some_tweet.text, User.query.filter_by(id = some_tweet.user_id).first().username])
...
def get_longest_tweet():
    ...
    for some_tweet in all_tweets:
        my_length = 0
        for var in some_tweet.text:
            if var != ' ':
                my_length += 1
        if my_length > length_longest_tweet:
            legnth_longest_tweet = my_length
            longest_tweet = some_tweet
            user = User.query.filter_by(id = some_tweet.user_id).first()
